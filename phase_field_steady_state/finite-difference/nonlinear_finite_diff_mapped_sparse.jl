...
function j_auto!(J::SparseMatrixCSC, U)
    ...
    # equations for ϕ
    for i=2 : (NUM-1)
        J[i - 1, 1 : (2*NUM+1)] = SparseVector(ForwardDiff.gradient(
    ...
    # First order
    for i=2 : (NUM-1)
        J[(NUM-2) + i - 1, 1 : (2*NUM+1)] = SparseVector(ForwardDiff.gradient(
            U -> 1/α * (1 - x(i)^2) * (U[NUM + i + 1] - U[NUM + i - 1]) / (2 * h) + 
                                    U[2*NUM + 1] * (U[NUM + i] + 1/S * U[i] - 1/S), U
                                    ))
    end
    
  J[(2*(NUM-2) + 1) : (2*(NUM-2) + 5), 1 : (2*NUM+1)] = [
    # B.C. for ϕ
...
function j!(J::SparseMatrixCSC, U)
#U = rand(2*NUM + 1, 2*NUM + 1)
    interval_start = -1.
    interval_end = 1.

    h = (interval_end - interval_start) / (NUM - 1)
    z(i) = interval_start + (i-1) * h

    df_dphi_m(i) = (ϵ_0 / α)^2 * (1 - z(i)^2)^2 * -2 / h^2 + 
                    (1 - U[i]) * (U[i] - 1/2 - m(U[NUM + i])) + -U[i] * (U[i] - 1/2 - m(U[NUM + i])) + U[i] * (1 - U[i])
    df_dphi_r(i) = (ϵ_0 / α)^2 * (1 - z(i)^2)^2 / h^2 + (1 / α)^2 * (1 - z(i)^2) * (α * τ * U[2*NUM + 1] - 2 * ϵ_0^2 * z(i)) / (2 * h)
    df_dphi_l(i) = (ϵ_0 / α)^2 * (1 - z(i)^2)^2 / h^2 - (1 / α)^2 * (1 - z(i)^2) * (α * τ * U[2*NUM + 1] - 2 * ϵ_0^2 * z(i)) / (2 * h)
    df_dT(i) = -U[i] * (1 - U[i]) * m_prime(U[NUM + i])
    df_dc(i) = (1 - z(i)^2) * τ / α * (U[i+1] - U[i-1]) / (2 * h)

    dg_dphi = U[2*NUM + 1] / S
    dg_dT_m = U[2*NUM + 1]
    dg_dT_r(i) = 1 / α * (1 - z(i)^2) / (2 * h)
    dg_dT_l(i) = 1 / α * (1 - z(i)^2) / -(2 * h)
    dg_dc(i) = U[NUM + i] + (U[i] - 1) / S
    ...
    df_dc_vec = sparse([df_dc(i) for i=2:NUM-1])
    ...
    dg_dc_vec = sparse([dg_dc(i) for i=2:NUM-1])
#=
    E = spzeros(5, 2 * NUM + 1)
    E[1, 1] = 1
    E[2, NUM] = 1
...
=#
    ...
    J[:] = [A B df_dc_vec; C D dg_dc_vec; E]
end
